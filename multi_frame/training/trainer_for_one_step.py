...
from one_step_dataset_loader import OneStepDatasetsIteratorForCrossValidation
from network.lstm import LSTM, GRU, RNN, AttentionLSTM, AttentionGRU

class OneStepTrainerBase(object):
...
class CrossValidationTrainer(TrainerBase):
    ...
    def cross_validate(self):
        ...
        '''
        cross_validation_dataset_loader = OneStepDatasetsIteratorForCrossValidation(self.dataset_path, self.split_num, xp=self.xp, test_ids=self.test_ids, train_ids=self.train_ids, iterator=self.data_iterator)

        for train_iterator, val_iterator in cross_validation_dataset_loader:
