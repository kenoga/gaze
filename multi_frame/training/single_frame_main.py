...
####  Parameter Settings :)
networks = [OneLayerFeedForwardNeuralNetwork, TwoLayerFeedForwardNeuralNetwork]
batch_sizes = [64]
...
for network in networks:
    for batch_size in batch_sizes:
        for dataset_path_and_rnn_input in dataset_path_and_rnn_inputs:
            ...
            config_id =  "%s_%s_%04d_%02d" % \
                            (network.name, input_type, batch_size, len(train_ids)-1)

            network_params = {"n_in": nn_input, "n_out": nn_output}

            SingleFrameDataIterator.set_params(batch_size, xp=cp)
